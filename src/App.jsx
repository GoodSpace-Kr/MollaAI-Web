      ...
      >
        <div className="p-8">
          ...
          <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            {activeTab === 'signup' && (
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-2 ml-1">이름</label>
                <input 
                  type="text" 
                  placeholder="홍길동"
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-2 ml-1">이메일</label>
              <input 
                type="email" 
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-2 ml-1">비밀번호</label>
              <input 
                type="password" 
                placeholder="••••••••"
                className="w-full px-4 py-3 bg-surface rounded-xl border border-transparent focus:border-primary focus:bg-white transition-all outline-none"
...
const FAQItem = ({ question, answer }) => {
  ...
  return (
    <div className="border-b border-surface-container last:border-0">
      ...
      >
        <span className="text-lg font-semibold text-on-surface group-hover:text-primary transition-colors">{question}</span>
        {isOpen ? <ChevronUp className="text-primary" /> : <ChevronDown className="text-on-surface-variant" />}
      </button>
      <AnimatePresence>
        {isOpen && (
          <motion.div 
    ...
    <div className="min-h-screen bg-surface selection:bg-primary/20 font-sans antialiased">
      ...
      <nav 
        className={`fixed top-0 w-full z-50 transition-all duration-300 ${
          isScrolled ? "bg-surface/80 backdrop-blur-md shadow-sm py-2" : "bg-transparent py-4"
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-4 lg:gap-8 mx-4">
            {["서비스 소개", "학습 코스", "요금", "후기", "FAQ", "위치", "문의하기"].map((item) => (
              <a 
                key={item} 
                href={`#${item}`}
            ...
            >
              <div className="p-6 flex flex-col gap-4">
                ...
                <div className="grid grid-cols-2 gap-4 mt-2">
                  <button onClick={() => {openLogin(); setMobileMenuOpen(false);}} className="bg-surface text-on-surface py-3 rounded-xl font-bold">
                    로그인
                  </button>
                  <button onClick={() => {openSignup(); setMobileMenuOpen(false);}} className="bg-primary text-on-primary py-3 rounded-xl font-bold">
                    시작하기
                  </button>
                </div>
              </div>
      ...
      <main>
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            >
              ...
              <p className="text-base md:text-lg lg:text-xl text-on-surface-variant max-w-lg leading-relaxed mx-auto lg:mx-0">
                ...
                자연스럽게 입을 열어보세요. 당신의 속도와 관심사에 100% 맞춰줍니다.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 pt-4 justify-center lg:justify-start">
                <button onClick={openSignup} className="bg-primary text-on-primary px-10 py-5 rounded-2xl font-bold text-lg shadow-xl shadow-primary/20 hover:-translate-y-1 transition-transform flex items-center justify-center gap-2 group">
                  무료 체험 시작하기
              ...
              <div className="flex items-center gap-4 text-xs md:text-sm text-on-surface-variant font-medium justify-center lg:justify-start">
                <div className="flex -space-x-2">
                  {[1,2,3,4].map(i => (
                    <div key={i} className="w-8 h-8 rounded-full border-2 border-white bg-surface-container" />
                  ))}
                </div>
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-on-surface-variant">말하기 학습 중...</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div className="bg-surface p-4 rounded-2xl rounded-tl-none text-sm leading-relaxed max-w-[80%]">
                      "Hello! How was your day today? Would you like to practice ordering at a cafe?"
                    </div>
                    <div className="bg-primary/10 p-4 rounded-2xl rounded-tr-none text-sm leading-relaxed ml-auto max-w-[80%] text-right font-medium">
                      "I'm good! Yes, I want to practice that. Can I have a latte?"
                    </div>
                    <div className="bg-surface p-4 rounded-2xl rounded-tl-none text-sm leading-relaxed max-w-[80%]">
                      "Great start! Instead of 'Can I have', you could also say 'I'd like to order' to sound more natural."
                    </div>
                  </div>
                  <div className="pt-4 border-t border-surface-container flex justify-between items-center">
                    ...
                    <Phone className="text-primary animate-pulse" fill="currentColor" />
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        {/* Problems Section */}
        <section id="서비스 소개" className="bg-surface-container-low py-24 md:py-32 px-6 md:px-12">
          <div className="max-w-7xl mx-auto">
            <motion.div {...fadeIn} className="text-center space-y-4 mb-16 md:mb-24">
              <span className="text-primary font-bold tracking-[0.2em] text-sm uppercase">The Challenge</span>
              <h2 className="text-3xl md:text-5xl font-bold text-on-surface">왜 우리는 영어 앞에서 작아질까요?</h2>
              <p className="text-on-surface-variant max-w-2xl mx-auto text-lg">기존의 학습 방식이 당신과 맞지 않았을 뿐입니다.</p>
            </motion.div>
        ...
        {/* Features Section */}
        <section id="학습 코스" className="py-24 md:py-32 px-6 md:px-12 max-w-7xl mx-auto">
          <motion.div {...fadeIn} className="text-center space-y-4 mb-20 md:mb-32">
            <span className="text-primary font-bold tracking-[0.2em] text-sm uppercase">Curriculum</span>
            <h2 className="text-3xl md:text-5xl font-bold text-on-surface">당신에게 필요한 모든 상황</h2>
            <p className="text-on-surface-variant max-w-2xl mx-auto text-lg">왕초보부터 비즈니스까지, 500개 이상의 테마별 코스</p>
          </motion.div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {[
              ...
              >
                <h4 className="text-xl font-bold mb-2 group-hover:text-primary transition-colors">{course.title}</h4>
        ...
        {/* Pricing Section */}
        <section id="요금" className="py-24 md:py-32 px-6 md:px-12 bg-surface-container-low">
          <div className="max-w-7xl mx-auto">
            <motion.div {...fadeIn} className="text-center space-y-6 mb-16">
              <span className="text-primary font-bold tracking-[0.2em] text-sm uppercase">Pricing Plans</span>
              <h2 className="text-3xl md:text-5xl font-bold text-on-surface">합리적인 비용으로 시작하세요</h2>
              
              <div className="flex items-center justify-center gap-4 pt-4">
                <span className={`text-sm font-bold ${billingCycle === 'monthly' ? 'text-primary' : 'text-on-surface-variant'}`}>월간 결제</span>
                <button 
                ...
                <span className={`text-sm font-bold ${billingCycle === 'yearly' ? 'text-primary' : 'text-on-surface-variant'}`}>
                  연간 결제 <span className="ml-1 text-[10px] bg-primary text-white px-1.5 py-0.5 rounded-full">20% 할인</span>
                </span>
              </div>
            </motion.div>
              ...
              {[
                {
                  ...
                  features: ["월 30분 통화", "기본 AI 튜터 제공", "학습 리포트 제공", "메신저 알림"],
                  popular: false
                },
                {
                  name: "프로",
                  ...
                  features: ["월 100분 통화", "프리미엄 AI 튜터 제공", "상세 문법 교정 리포트", "맞춤형 학습 커리큘럼", "우선 고객 지원"],
                  popular: true
                },
                {
                  name: "비즈니스",
                  ...
                  features: ["무제한 통화", "모든 AI 튜터 모델 사용", "실시간 발음 코칭", "비즈니스 전문 과정 포함", "1:1 전담 매니저 상담"],
                  popular: false
                }
              ].map((plan, i) => (
                <motion.div 
                ...
                >
                  ...
                  <div className="flex items-baseline gap-1">
                    ...
                    <span className="text-on-surface-variant font-medium">/월</span>
                  </div>
                  <button className={`w-full py-4 rounded-2xl font-bold transition-all ${plan.popular ? 'bg-primary text-on-primary shadow-lg shadow-primary/20 hover:scale-[1.02]' : 'bg-surface text-on-surface hover:bg-surface-container'}`}>
                    요금제 선택하기
                  </button>
        ...
        {/* Reviews Section */}
        <section id="후기" className="bg-on-surface py-24 md:py-32 px-6 md:px-12 text-white overflow-hidden">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-end gap-8 mb-16 md:mb-24">
              <motion.div {...fadeIn} className="space-y-4">
                <span className="text-primary font-bold tracking-[0.2em] text-sm uppercase">Testimonials</span>
                <h2 className="text-3xl md:text-5xl font-bold text-white">이미 많은 분들이 <br className="hidden lg:block" />변화를 경험했습니다.</h2>
              </motion.div>
              <div className="flex gap-4 md:gap-8">
                <div className="text-center">
                  <div className="text-4xl font-bold text-primary">4.9/5</div>
                  <div className="text-sm text-on-surface-variant">사용자 만족도</div>
                </div>
                <div className="text-center">
                  <div className="text-4xl font-bold text-primary">50만+</div>
                  <div className="text-sm text-on-surface-variant">누적 통화 건수</div>
                </div>
              </div>
            </div>
              ...
              {[
                ...
                >
                  <div className="flex gap-1">
                    {[...Array(review.stars)].map((_, i) => <Star key={i} size={16} className="fill-primary text-primary" />)}
                  </div>
                  <p className="text-lg leading-relaxed text-surface-container">"{review.text}"</p>
                  <div className="flex items-center gap-4 pt-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-on-surface-variant">{review.role}</div>
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        <section id="FAQ" className="py-24 md:py-32 px-6 md:px-12 max-w-4xl mx-auto">
          <motion.div {...fadeIn} className="text-center space-y-4 mb-16">
            <span className="text-primary font-bold tracking-[0.2em] text-sm uppercase">Support</span>
            <h2 className="text-3xl md:text-5xl font-bold text-on-surface">자주 묻는 질문</h2>
          </motion.div>
          
          <motion.div {...fadeIn} className="bg-white rounded-[32px] p-8 md:p-12 shadow-xl shadow-primary/5 border border-surface-container">
            <FAQItem 
        ...
        {/* Location Section */}
        <section id="위치" className="bg-surface-container-low py-24 md:py-32 px-6 md:px-12">
          <div className="max-w-7xl mx-auto">
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
              <motion.div {...fadeIn} className="space-y-6 md:space-y-8">
                <span className="text-primary font-bold tracking-[0.2em] text-sm uppercase">Visit Us</span>
                <h2 className="text-3xl md:text-5xl font-bold text-on-surface">찾아오시는 길</h2>
                <p className="text-on-surface-variant text-lg leading-relaxed">
                  Molla AI 헤드오피스는 인천의 중심, 부평역 인근에 위치하고 있습니다. <br />
                ...
                <div className="space-y-6">
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-on-surface-variant">인천광역시 부평구 부평역 광장로 1</div>
                    </div>
                  </div>
                  <div className="flex gap-4">
                    <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-primary shadow-sm">
                    ...
                    <div>
                      ...
                      <div className="text-on-surface-variant">070-1234-5678</div>
                    </div>
                  </div>
                  <div className="flex gap-4">
                    <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-primary shadow-sm">
                    ...
                    <div>
                      ...
                      <div className="text-on-surface-variant">hello@molla.ai</div>
                    </div>
                  </div>
                </div>
              </motion.div>
        ...
        <section id="문의하기" className="py-24 md:py-32 px-6 md:px-12">
          <div className="max-w-4xl mx-auto">
            <motion.div {...fadeIn} className="bg-white rounded-[40px] p-8 md:p-16 shadow-2xl shadow-primary/5 border border-surface-container">
              <div className="text-center space-y-4 mb-12">
                <h2 className="text-3xl md:text-5xl font-bold">문의하기</h2>
                <p className="text-on-surface-variant">비즈니스 제휴, 기업 도입 문의 등 무엇이든 물어보세요.</p>
              </div>
              ...
              <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-2 ml-1">이름</label>
                    <input type="text" className="w-full px-5 py-4 bg-surface rounded-2xl border border-transparent focus:border-primary focus:bg-white transition-all outline-none" placeholder="성함을 입력하세요" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-2 ml-1">이메일</label>
                    <input type="email" className="w-full px-5 py-4 bg-surface rounded-2xl border border-transparent focus:border-primary focus:bg-white transition-all outline-none" placeholder="email@example.com" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-2 ml-1">문의 내용</label>
                  <textarea rows={5} className="w-full px-5 py-4 bg-surface rounded-2xl border border-transparent focus:border-primary focus:bg-white transition-all outline-none resize-none" placeholder="문의 내용을 자세히 적어주세요"></textarea>
                </div>
        ...
        <section className="py-24 md:py-32 px-6 md:px-12 bg-primary">
          ...
          >
            ...
            <div className="pt-4 flex flex-col sm:flex-row gap-6 justify-center">
              <button onClick={openSignup} className="bg-white text-primary px-12 py-6 rounded-2xl font-bold text-xl hover:scale-105 transition-all shadow-xl shadow-black/10">
                무료로 첫 전화 걸어보기
              </button>
              <button 
      ...
      <footer className="bg-surface py-20 px-6 md:px-12 border-t border-surface-container">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          <div className="space-y-6">
            ...
            <p className="text-on-surface-variant text-sm leading-relaxed">
              우리는 누구나 영어로 자유롭게 <br /> 꿈꿀 수 있는 세상을 만듭니다. <br />
          ...
          {[
            { title: "제품", links: ["서비스 소개", "학습 코스", "요금제", "자주 묻는 질문", "기업 도입"] },
            { title: "회사", links: ["회사 소개", "채용 공고", "블로그", "브랜드 가이드"] },
            { title: "법률", links: ["이용약관", "개인정보처리방침", "환불규정", "청소년보호정책"] }
          ].map((col, i) => (
            <div key={i} className="space-y-6">
              <h4 className="font-bold text-on-surface uppercase tracking-widest text-xs">{col.title}</h4>
              <ul className="space-y-4">
                {col.links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-on-surface-variant hover:text-primary text-sm font-medium transition-colors">
                      {link}
                    </a>
                  </li>
                ))}
        ...
        <div className="max-w-7xl mx-auto mt-20 pt-8 border-t border-surface-container flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-8">
            <a href="#" className="text-xs text-on-surface-variant/40 hover:text-primary transition-colors font-medium">개인정보처리방침</a>
            <a href="#" className="text-xs text-on-surface-variant/40 hover:text-primary transition-colors font-medium">이용약관</a>
          </div>
        </div>
      </footer>
    </div>
